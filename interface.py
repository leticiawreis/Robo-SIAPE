...
class Worker(QThread):
    ...
    def run(self):
        ...
        com os caminhos gerados; se falhar, registra a exceção no log e
        coloca um evento "error" na fila com a mensagem de erro. Ao
        final, aguarda a thread de repasse de log terminar (com timeout).
        """
...
class App(QMainWindow):
    ...
    def tela_execucao(self, ano, mes, tipo):
        """Monta e exibe a tela de acompanhamento de execução, e dispara
        de fato a execução do robô em uma thread separada (Worker).

        Monta: cabeçalho com o status "PROCESSANDO"; cartão com o
        período/tipo selecionado e uma dica sobre o funcionamento do
        download; um console de log (QTextEdit somente leitura) que é
        alimentado em tempo real pelos sinais do Worker; e um rodapé com
        o botão "Voltar ao painel" (inicialmente desabilitado até a
        execução terminar) e o rótulo de resultado.

        Cria e inicia o Worker com os parâmetros informados, conectando
        seus sinais (log_signal, done_signal, error_signal) aos métodos
        que atualizam a interface (_log_gui, _execucao_concluida,
        _execucao_erro).
    ...
    def _log_gui(self, texto):
        ...
        log_signal) e a acrescenta ao console de log da tela de
        execução, atualizando também o contador de eventos exibido.
    ...
    def _execucao_concluida(self, caminho, caminho_log):
        ...
        Atualiza o indicativo de status para "CONCLUÍDO", registra
        linhas finais no log da interface (caminhos da planilha e do
        log gerados), habilita o botão de voltar ao painel, registra a
        execução no histórico (registrar_execucao) e, conforme as
        preferências do usuário, abre a pasta de saída automaticamente
        e/ou exibe uma notificação de conclusão.
    ...
    def _execucao_erro(self, erro):
        ...
        Atualiza o indicativo de status para "ERRO", registra a
        mensagem de erro no log da interface, habilita o botão de voltar
        ao painel, registra a falha no histórico (registrar_execucao com
        status "erro") e, se a preferência de notificação estiver
        ativa, exibe uma mensagem de erro ao usuário.
    ...
    def tela_execucao_placeholder(self):
        ...
        andamento, não faz nada (evita iniciar duas execuções
        simultâneas); caso contrário, encaminha para a tela de
