...
class ExecucaoCancelada(Exception):
    """Levantada internamente quando o usuário solicita a interrupção do
    pipeline em andamento (via botão "Parar" na interface).

    A parada é sempre cooperativa: o pipeline não é interrompido à
    força no meio de uma operação sensível (ex: escrita de arquivo);
    em vez disso, o código verifica periodicamente `evento_parar` (um
    `threading.Event` compartilhado com a interface) e levanta esta
    exceção assim que atinge um ponto seguro para parar — normalmente
    entre blocos do download, ou entre tentativas.
    """
...
def meses_disponiveis(ano: str) -> list[str]:
...
def criar_pasta_execucao(ano: str, mes: str) -> str:
...
def limpar_pasta_trabalho(pasta: str | None) -> None:
...
def baixar_pacote(
    ...
    evento_parar: "threading.Event | None" = None,
) -> str:
    ...
    Parâmetros:
        ...
        evento_parar (threading.Event, opcional): se fornecido, é
            verificado a cada bloco baixado; assim que for sinalizado
            (usuário clicou em "Parar" na interface), o download é
            interrompido, o arquivo parcial é descartado e
            ExecucaoCancelada é levantada.
    ...
    Levanta:
        ...
        ExecucaoCancelada: se o usuário solicitar a parada da execução
            (via evento_parar) durante o download.
    """
                ...
                with open(caminho_parcial, "wb") as arquivo:
                    for bloco in resposta.iter_content(chunk_size=512 * 1024):
                        if evento_parar is not None and evento_parar.is_set():
                            ...
                            raise ExecucaoCancelada("Download cancelado pelo usuário.")
...
def baixar_com_retentativas(
    ...
    Se `evento_parar` for sinalizado (usuário clicou em "Parar" na
    interface), a espera entre tentativas é interrompida imediatamente
    e nenhuma nova tentativa é feita — a pasta de trabalho é limpa e
    ExecucaoCancelada é levantada.
    ...
    Parâmetros:
        ...
        evento_parar (threading.Event, opcional): sinalizador
            compartilhado de parada solicitada pelo usuário.
    ...
    for tentativa in range(1, MAX_TENTATIVAS_DOWNLOAD + 1):
        if evento_parar is not None and evento_parar.is_set():
            ...
            logger.info("Execução cancelada pelo usuário antes do download.")
        ...
        try:
            ...
            arquivo = baixar_pacote(
                ano, mes, tipo, pasta_download, logger, evento_parar
            )
            ...
            if evento_parar is not None and evento_parar.wait(2):
                limpar_pasta_trabalho(pasta_download)
                logger.info("Execução cancelada pelo usuário.")
                raise ExecucaoCancelada("Execução cancelada pelo usuário.") from erro
            elif evento_parar is None:
                time.sleep(2)
...
# =====================================================================
def encontrar_arquivo_remuneracao(pasta_extraida: str) -> str:
...
def preparar_csv(caminho: str) -> tuple["TextIO", "csv._reader"]:
...
def coluna_vazia(valor: object) -> bool:
...
def identificar_colunas_validas(
    caminho_csv: str,
    logger: logging.Logger,
    evento_parar: "threading.Event | None" = None,
    ...
    try:
        ...
        for linha in leitor:
            # A identificação das colunas pode percorrer centenas de milhares
            # de registros. Verificamos periodicamente se o usuário pediu para
            # parar, para que o botão "Parar" também funcione nesta etapa.
            if total_linhas % 1000 == 0:
                if evento_parar is not None and evento_parar.is_set():
                    logger.info(
                        "Execução interrompida pelo usuário durante a análise do CSV."
                    )
                    raise ExecucaoCancelada(
                        "Execução cancelada durante a análise do CSV."
                    )
...
def converter_valor_monetario(valor: object) -> float | None:
...
def ler_linhas_tratadas(
    caminho_csv: str, colunas_validas: list[int]
...
def identificar_colunas_monetarias(cabecalho: list[str]) -> set[int]:
...
def calcular_largura_colunas(cabecalho: list[str]) -> list[int]:
...
def extrair_zip(caminho_zip: str, pasta_extraida: str, logger: logging.Logger) -> None:
...
def remover_arquivos_desnecessarios(pasta_extraida: str, logger: logging.Logger) -> None:
...
def validar_periodo(ano: str, mes: str, tipo: str | None = None) -> None:
...
def limpar_arquivos_extraidos(pasta: str | None) -> None:
...
def executar_pipeline_completo(
    ...
    Parâmetros:
        ...
        log_queue (queue.Queue): fila usada para repassar mensagens de
            ...
            interface gráfica).
        evento_parar (threading.Event, opcional): sinalizador
            compartilhado com a interface gráfica; quando setado
            (usuário clicou em "Parar"), a execução é interrompida no
            próximo ponto seguro de verificação, levantando
            ExecucaoCancelada.
    ...
    try:
        ...
        if evento_parar is not None and evento_parar.is_set():
            logger.info("Execução cancelada pelo usuário após o download.")
            raise ExecucaoCancelada("Execução cancelada pelo usuário.")
        ...
        if evento_parar is not None and evento_parar.is_set():
            logger.info("Execução cancelada pelo usuário após a extração.")
            raise ExecucaoCancelada("Execução cancelada pelo usuário.")
        ...
        if evento_parar is not None and evento_parar.is_set():
            logger.info("Execução cancelada pelo usuário antes da geração da planilha.")
            raise ExecucaoCancelada("Execução cancelada pelo usuário.")
